...
let pareNumbers = random.filter((el) => el % 2 === 0 );
...
function incrementation() {
  let numeriIncrementati = random.map((el) => el * 2)
...
const stringhe = ['EPICODE', 'fa', 'cagare']

function stringsLength () {
  let len = stringhe.map((el) => el.length);
  return len;
}
...
console.log(filmTotali());

/* ESERCIZIO 11 (map)
  Scrivi una funzione per creare un array con solamente i titoli dei film contenuti nell'array fornito.
*/

const titoli = () => movies.map((el) => el.Title);
console.log(titoli());
...
const after2000 = movies.filter((el) => el.Year > 2000);
